  ...
  return (
    <div className="checkBox-group mt-20">
      <div className="form-group">
        {label && <label className="labelClass margin-left-5">{label}{required && <span className="color-red">*</span>}</label>}
            ...
            <div className="form-check ml-30" key={key}>
              <input disabled={disabled} onClick={(e) => {
                ...
                onChange(values);
                     ...
                     className="form-check-input" type="checkbox" value={option.value} checked={option.selected}
                     id={"flexCheckDefault"+key}/>
              <label className="form-check-label inputClassLabel mt-5px" htmlFor={"flexCheckDefault"+key}>
                {option.label}
              </label>
            </div>
          )
